  ...
  return (
    <SidebarMenu>
      <SidebarMenuItem>
        <SidebarMenuButton
          size="sm"
          className={`data-[state=open]:bg-secondary w-full justify-start ${className || ''}`}
        >
          <div className="mr-2">
            <Image
              src={`/roles/${selectedRole || 'end_user'}.svg`}
              alt="Role Icon"
              width={24}
              height={24}
            />
          ...
          <div className="grid flex-1 text-left text-sm leading-tight">
            <span className="font-medium">
              {selectedRole ? selectedRole.replace('_', ' ') : 'Select Role'}
            </span>
          </div>
        </SidebarMenuButton>
      </SidebarMenuItem>
