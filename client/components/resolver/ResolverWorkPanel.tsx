...
import { Play, CheckCircle2, MessageSquare, Info } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      ...
      {ticket.status === 'Open' && canWork && (
        <Button className="w-full" onClick={handleStartWork} disabled={isProcessing}>
          {isProcessing ? (
          ...
          {showInfoForm && (
            <div className="space-y-2 p-3 bg-blue-50 rounded-xl border border-blue-100">
              <p className="text-xs text-blue-700 flex items-center gap-1.5">
                ...
                Ticket will be paused and marked as awaiting info. Optionally add a message for the
                user.
              </p>
              <Textarea
              ...
              <div className="flex gap-2 justify-end">
                <Button variant="ghost" size="sm" onClick={() => setShowInfoForm(false)}>
                  Cancel
                </Button>
                <Button size="sm" onClick={handleRequestInfo} disabled={isProcessing}>
      ...
      {(ticket.status === 'Resolved' || ticket.status === 'Closed') && (
        <p className="text-sm text-center text-gray-400 py-2">This ticket has been resolved.</p>
      )}
    </div>
  );
}
