...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
      ...
      if (status === 'authenticated' && session?.user) {
        try {
          if (session.accessToken) {
            ...
            document.cookie = `accessToken=${session.accessToken}; ${Object.entries(COOKIE_OPTIONS)
    ...
    const handleSignOut = () => {
      ...
      signOut({ callbackUrl: window.location.pathname + window.location.search, redirect: true });
    };
