...
export default function ManagerDashboard() {
  ...
  const categoryRows = useMemo(() => {
    ...
    return categoryCounts
      .map((c) => ({ ...c, name: nameMap.get(c.categoryId) ?? `Category ${c.categoryId.slice(-4)}` }))
          ...
          <Card>
            ...
            <CardContent>
              {isLoading ? (
                ...
                const pageRows = resolverKpis.slice(kpiPage * KPI_PAGE_SIZE, (kpiPage + 1) * KPI_PAGE_SIZE);
                return (
                  <>
                    ...
                    {totalPages > 1 && (
                      <div className="flex items-center justify-between mt-4 pt-3 border-t">
                        <span className="text-xs text-muted-foreground">
                          {kpiPage * KPI_PAGE_SIZE + 1}–{Math.min((kpiPage + 1) * KPI_PAGE_SIZE, resolverKpis.length)} of {resolverKpis.length}
                        </span>
