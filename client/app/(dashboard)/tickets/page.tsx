...
const MANAGER_TABS: Queue[] = ["all", "open", "unassigned", "mine", "inprogress", "escalated", "resolved"];
...
export default function TicketsPage() {
  ...
  const filtered = useMemo(() => {
    switch (activeQueue) {
      case "open":       return allAvailable.filter((t) => t.status === "Open");
      case "unassigned": return allAvailable.filter((t) => t.status === "Open" && !t.assignedToId);
      case "mine":       return allAvailable.filter((t) => t.assignedToId === user?.id);
      case "inprogress": return allAvailable.filter((t) => t.status === "InProgress");
      case "waiting":    return allAvailable.filter((t) => t.status === "OnHold" || t.status === "AwaitingInfo");
      case "escalated":  return allAvailable.filter((t) => t.isEscalated);
      case "resolved":   return allAvailable.filter((t) => t.status === "Resolved" || t.status === "Closed");
      default:           return allAvailable;
    }
  ...
  const counts = useMemo(() => ({
    ...
    waiting:    allAvailable.filter((t) => t.status === "OnHold" || t.status === "AwaitingInfo").length,
  ...
  return (
    <div className="container mx-auto py-6 space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {isManager ? "Full ticket visibility across all agents" : "Your queue and unassigned tickets"}
          </p>
        </div>
      ...
      {isManager && (
        <TicketAnalyticsCards tickets={allAvailable} isLoading={isLoading} />
      )}
