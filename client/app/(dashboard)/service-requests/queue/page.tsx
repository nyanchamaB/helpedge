...
const accentMap: Record<QueueTab, { bg: string; icon: string }> = {
  ...
  "pending-approval": { bg: "bg-amber-50 dark:bg-amber-950/30", icon: "text-amber-600 dark:text-amber-400" },
...
export default function ServiceRequestQueuePage() {
  ...
  const columns: DataTableColumn<ServiceRequest>[] = [
    ...
    },
    {
      ...
      sortable: true,
      render: (r) => (
        <span className={cn("text-sm whitespace-nowrap", r.isOverdue && "text-red-600 font-medium")}>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stats cards */}
      <div className={`grid grid-cols-2 gap-3 ${canSeeUnassigned ? "sm:grid-cols-4" : "sm:grid-cols-3"}`}>
        {visibleTabs.map((tab) => {
      ...
      <DataTable<ServiceRequest>
        ...
        actions={actions}
        pagination={true}
        onRowClick={(r) => navigateTo(`/service-requests/${r.id}`, { from: '/service-requests/queue' })}
        getItemId={(r) => r.id}
        emptyState={{
