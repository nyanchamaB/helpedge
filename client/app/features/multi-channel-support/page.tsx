...
const CHANNELS = [
  {
    ...
    desc: 'Every inbound email auto-converts to a tracked ticket with full thread history.',
  },
  {
    icon: <MessageSquare className="w-5 h-5" />,
    label: 'Live Chat',
    color: 'bg-emerald-50 dark:bg-emerald-950 text-emerald-600',
    desc: 'Embedded chat widget on your portal or website. Agents see full context before they respond.',
  },
  {
    icon: <Phone className="w-5 h-5" />,
    label: 'Phone',
    color: 'bg-violet-50 dark:bg-violet-950 text-violet-600',
    desc: 'Call logs linked to tickets automatically. No more lost verbal commitments.',
  },
  {
    icon: <Globe className="w-5 h-5" />,
    label: 'Web Portal',
    color: 'bg-amber-50 dark:bg-amber-950 text-amber-600',
    desc: 'Self-service portal submissions routed alongside all other channels in one unified queue.',
  },
  {
    icon: <Slack className="w-5 h-5" />,
    label: 'Slack / Teams',
    color: 'bg-pink-50 dark:bg-pink-950 text-pink-600',
    desc: 'Employees raise tickets and get updates without leaving their collaboration tool.',
  },
  {
    icon: <Smartphone className="w-5 h-5" />,
    label: 'Mobile App',
    color: 'bg-sky-50 dark:bg-sky-950 text-sky-600',
    desc: 'Native iOS and Android app for submitting, tracking, and commenting on requests on the go.',
  },
];
...
const TESTIMONIAL = {
  quote:
    'Before HelpEdge, our agents were juggling five different inboxes. Now everything is in one place and our response time dropped from 6 hours to 47 minutes.',
  author: 'Ashley Merley',
  role: 'Service Desk Lead, EastAfrica Logistics',
  initials: 'AM',
};

// ✅ Extracted from inline map — hooks cannot be called inside callbacks
...
const PROOF_CARDS: ProofCardItem[] = [
  {
    ...
    title: '1,840 emails processed this month',
    sub: 'Zero missed · avg 38min response',
  },
  {
    icon: <Slack className="w-5 h-5 text-pink-600" />,
    title: 'Slack tickets up 3× since launch',
    sub: 'Employees prefer it over email',
  },
  {
    icon: <MessageSquare className="w-5 h-5 text-emerald-600" />,
    title: 'Chat resolves in < 8 minutes avg',
    sub: 'Fastest channel for tier-1 issues',
  },
  {
    icon: <BarChart2 className="w-5 h-5 text-violet-600" />,
    title: '92% first-contact resolution rate',
    sub: 'Up from 61% before unification',
  },
];

function ProofCard({ item, index }: { item: ProofCardItem; index: number }) {
  const { ref, visible } = useFadeIn(); // ✅ hook at component top level
  ...
  const tickets = [
    {
      id: 'INC-3041',
      channel: 'email',
      channelIcon: <Mail className="w-3 h-3" />,
      channelColor: 'text-blue-600 bg-blue-50 dark:bg-blue-950',
      from: 'grace.njoroge@company.com',
      subject: 'Cannot access shared drive',
      time: '2m ago',
      status: 'New',
      statusColor: 'bg-red-100 text-red-600 dark:bg-red-950',
    },
    {
      id: 'INC-3040',
      channel: 'slack',
      channelIcon: <Slack className="w-3 h-3" />,
      channelColor: 'text-pink-600 bg-pink-50 dark:bg-pink-950',
      from: '#it-support',
      subject: 'Printer offline on 3rd floor',
      time: '8m ago',
      status: 'Open',
      statusColor: 'bg-amber-100 text-amber-600 dark:bg-amber-950',
    },
    {
      id: 'INC-3039',
      channel: 'chat',
      channelIcon: <MessageSquare className="w-3 h-3" />,
      channelColor: 'text-emerald-600 bg-emerald-50 dark:bg-emerald-950',
      from: 'james.mwangi',
      subject: 'VPN keeps disconnecting',
      time: '14m ago',
      status: 'In Progress',
      statusColor: 'bg-blue-100 text-blue-600 dark:bg-blue-950',
    },
    {
      id: 'INC-3038',
      channel: 'phone',
      channelIcon: <Phone className="w-3 h-3" />,
      channelColor: 'text-violet-600 bg-violet-50 dark:bg-violet-950',
      from: '+254 712 345 678',
      subject: 'Software licence expired',
      time: '31m ago',
      status: 'Resolved',
      statusColor: 'bg-emerald-100 text-emerald-600 dark:bg-emerald-950',
    },
  ];
        ...
        <div className="flex h-72">
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-1.5">
                    <span
                      className={`flex items-center justify-center w-4 h-4 rounded ${t.channelColor}`}
                    >
                      {t.channelIcon}
                    </span>
          ...
          <div className="flex-1 p-4 flex flex-col">
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${tickets[active].statusColor}`}
              >
            ...
            <div className="flex-1 bg-muted/40 rounded-xl p-3 mb-3 space-y-2">
              <div className="flex gap-2">
                ...
                <div className="bg-background rounded-lg px-2.5 py-1.5 text-[10px] max-w-[80%]">
                  Hi, I can&rsquo;t seem to access the shared drive. Getting a permission error
                  since this morning.
                </div>
              </div>
              <div className="flex gap-2 justify-end">
                <div className="bg-blue-600 text-white rounded-lg px-2.5 py-1.5 text-[10px] max-w-[80%]">
                  Thanks for reporting. Checking your permissions now — will update you in 10
                  minutes.
                </div>
            ...
            <div className="flex gap-2">
              <input
                className="flex-1 text-[10px] border border-border rounded-lg px-2.5 py-1.5 bg-background"
                placeholder="Reply via email..."
                readOnly
              />
              <button className="bg-blue-600 text-white text-[10px] px-3 py-1.5 rounded-lg font-semibold">
                Send
              </button>
            </div>
          </div>
...
function ChannelCard({ ch, index }: { ch: (typeof CHANNELS)[0]; index: number }) {
  ...
  return (
    ...
    >
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${ch.color} group-hover:scale-110 transition-transform duration-300`}
      >
        {ch.icon}
      </div>
  ...
  return (
    <div className="border-b border-border last:border-0">
      ...
      >
        {faq.q}
        <ChevronDown
          className={`w-4 h-4 shrink-0 ml-4 transition-transform duration-300 ${open ? 'rotate-180 text-blue-600' : 'text-muted-foreground'}`}
        />
      </button>
      <div
      ...
      <main className="overflow-hidden">
        ...
        <section className="relative max-w-7xl mx-auto px-6 pt-20 pb-24 grid md:grid-cols-2 gap-16 items-center">
          ...
          <div className="relative space-y-6">
            ...
            <p className="text-muted-foreground text-lg leading-relaxed max-w-md">
              Email, chat, Slack, Teams, phone, portal — every channel unified into one intelligent
              inbox. No ticket falls through the cracks.
            </p>
              ...
              {[
                {
                  ...
                  text: '6 channels out of the box',
                },
                {
        ...
        <section className="border-y border-border bg-muted/30 py-16">
          <div className="max-w-4xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-10">
            {/* ✅ StatCard component — hook called at component level */}
            {STATS.map((s, i) => (
        ...
        <section className="max-w-7xl mx-auto px-6 py-24">
          <div className="text-center mb-14">
            ...
            <p className="text-muted-foreground max-w-xl mx-auto">
              Stop asking employees to use a specific channel. Support them on the one they already
              use.
            </p>
          </div>
          ...
          <div className="max-w-5xl mx-auto px-6">
            <div className="text-center mb-14">
              <h2 className="text-3xl md:text-4xl font-black mb-3">
                Built for agents, not just managers.
              </h2>
              <p className="text-muted-foreground max-w-xl mx-auto">
        ...
        <section className="max-w-5xl mx-auto px-6 py-24 grid md:grid-cols-2 gap-16 items-center">
          ...
          {/* ✅ ProofCard component — hook called at component level */}
        ...
        <section className="bg-muted/30 border-y border-border py-24">
          <div className="max-w-3xl mx-auto px-6 text-center">
            <div className="text-5xl text-emerald-200 dark:text-emerald-900 font-serif mb-6">
              &ldquo;
            </div>
          ...
          <div className="relative rounded-3xl bg-gradient-to-br from-emerald-600 to-emerald-800 px-8 md:px-16 py-16 text-white overflow-hidden text-center">
            ...
            <div className="relative z-10 max-w-xl mx-auto">
              <h2 className="text-3xl md:text-4xl font-black mb-4">
                Stop managing five inboxes. Start managing one.
              </h2>
