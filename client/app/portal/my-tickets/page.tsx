  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <Button onClick={() => navigateTo("/portal/create-ticket", { from: '/portal/my-tickets' })} size="sm">
      ...
      {!isLoading && (
        <p className="text-xs text-muted-foreground -mt-3">
          {filtered.length === 0
            ? "No tickets found"
            : <>Showing <span className="font-medium text-foreground">{(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, filtered.length)}</span> of <span className="font-medium text-foreground">{filtered.length}</span></>
          }
          {hasFilters && (
            <button
      ...
      {isLoading ? (
        ...
        <EmptyState hasFilters={hasFilters} onCreateNew={() => navigateTo("/portal/create-ticket", { from: '/portal/my-tickets' })} onClear={() => { setSearch(""); setStatusFilter("all"); setPage(1); }} />
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {paged.map((ticket) => (
...
const accentMap: Record<string, { bg: string; icon: string; ring: string }> = {
  blue:   { bg: "bg-blue-50 dark:bg-blue-950/30",   icon: "text-blue-600 dark:text-blue-400",   ring: "ring-blue-500/40" },
  amber:  { bg: "bg-amber-50 dark:bg-amber-950/30", icon: "text-amber-600 dark:text-amber-400", ring: "ring-amber-500/40" },
  violet: { bg: "bg-violet-50 dark:bg-violet-950/30",icon: "text-violet-600 dark:text-violet-400",ring:"ring-violet-500/40" },
  green:  { bg: "bg-green-50 dark:bg-green-950/30", icon: "text-green-600 dark:text-green-400", ring: "ring-green-500/40" },
};
...
function Pagination({ page, totalPages, onChange }: {
  ...
  return (
    <div className="flex items-center justify-center gap-1.5 pt-2">
      <Button variant="outline" size="sm" disabled={page === 1} onClick={() => onChange(page - 1)} className="h-8 px-3 text-xs">
        Previous
      </Button>
      {pages.map((p, i) =>
        p === "…" ? (
      ...
      <Button variant="outline" size="sm" disabled={page === totalPages} onClick={() => onChange(page + 1)} className="h-8 px-3 text-xs">
        Next
      </Button>
    </div>
  );
...
function EmptyState({ hasFilters, onCreateNew, onClear }: {
  hasFilters: boolean;
  onCreateNew: () => void;
  onClear: () => void;
}) {
